/**
 * wolfir Hero — Premium white theme
 * Big-tech cloud security aesthetic
 */
import React from 'react';
import { motion } from 'framer-motion';
import { ArrowRight, Shield, Play, Zap } from 'lucide-react';

interface LandingHeroProps {
  /** 1-Click Demo: go to demo + run crypto-mining immediately */
  on1ClickDemo?: () => void;
}

const LandingHero: React.FC<LandingHeroProps> = ({ on1ClickDemo }) => {
  return (
    <section className="relative min-h-[90vh] flex items-center overflow-hidden bg-white">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-32 pb-24 w-full">
        <div className="text-center">
          ...
          >
            11,000 alerts/day. wolfir resolves them autonomously.
          </motion.p>
          ...
          >
            ...
            {on1ClickDemo && (
              <button
                onClick={(e) => { e.preventDefault(); on1ClickDemo(); }}
                className="inline-flex items-center justify-center gap-2 px-6 py-4 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-semibold text-base transition-all shadow-lg shadow-emerald-500/25 hover:shadow-emerald-500/35 border-2 border-emerald-500/50"
                title="Run crypto-mining demo immediately — no scenario pick needed"
              >
          ...
          {/* Two pillars — Cloud Security + AI Security */}
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            ...
            >
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-sm font-bold text-slate-900">Try Cloud Security Demo</span>
              </div>
              <p className="text-xs text-slate-600 leading-relaxed">
                Run crypto mining, privilege escalation, data exfiltration — full 5-agent pipeline with no AWS setup
              </p>
            </a>
            <a
              href="#demo"
            ...
            >
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-sm font-bold text-slate-900">Explore AI Security Posture</span>
              </div>
              <p className="text-xs text-slate-600 leading-relaxed">
                MITRE ATLAS, OWASP LLM Top 10, Shadow AI detection — see how we monitor our own pipeline
              </p>
            </a>
          </motion.div>

          {/* Bridge sentence — two-layer story */}
          <motion.p
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          >
            AI systems run on cloud infrastructure. When cloud is compromised, AI is compromised. wolfir closes both gaps — incident response plus AI pipeline monitoring. Credentials stay local.
          </motion.p>

          {/* Demo mode note — judges see full pipeline only with local backend */}
          <motion.p
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ delay: 0.55 }}
            className="mt-8 text-[11px] text-slate-400 max-w-lg mx-auto px-4 py-2 rounded-lg bg-slate-50 border border-slate-100"
          >
            Live demo uses instant client-side simulation when backend is offline. For the full Nova AI pipeline (5 agents, MITRE ATLAS, Agentic Query), run the backend locally.
          </motion.p>
